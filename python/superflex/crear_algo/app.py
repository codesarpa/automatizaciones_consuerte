from config import ENVIRONMENT,URL_SUPERFLEX,URL_HOME,USER,PASS, URL_GIROS_INTER, logs_diarios
...
logs = logs_diarios()
log = logs[0]
# URL_GIROS_INTER = "https://prod-superflex-admin.codesa.com.co/sf-admin-web-otros-convenios/admin/ho-giros-internacionales"
...
def ingresar_menu_giros_internacionales():
    try:
        ...
        page.goto(URL_GIROS_INTER, timeout=60000)
        page.wait_for_url(URL_GIROS_INTER, timeout=30000)  # Ajusta
        print("se ingresa al URL_GIROS_INTER")
        log.info("Se ingresa al modulo de URL_GIROS_INTER")
    except Exception as e:
        print(f"Error al ingresar URL_GIROS_INTER: {e}")
        log.error(f"Error al ingresar URL_GIROS_INTER: {e}")

def click_boton_crear():
    try:
        page.wait_for_timeout(10000)
        ...
        # page.goto(URL_GIROS_INTER, timeout=60000)
        # page.wait_for_url(URL_GIROS_INTER, timeout=30000)  # Ajusta
        print("se ingresa Crear")
        log.info("Se ingresa al modulo de Crear")
    except Exception as e:
        print(f"Error al ingresar URL_GIROS_INTER: {e}")
        log.error(f"Error al ingresar URL_GIROS_INTER: {e}")

def crear_terminales(cod_pdv, nom_pdv, terminal):
    page.wait_for_selector("app-loader", state="detached", timeout=60000)
    page.locator("button.p-button-icon-only").nth(0).click()
    print("se da click ")
...
if iniciar_sesion_superflex():
    ingresar_menu_giros_internacionales()
    click_boton_crear()


    
    #LEER ARCHIVO CSV 
    ARCHIVO = 'C:/automatizaciones_consuerte/python/superflex/crear_algo/terminales_meta.xlsx'
    df = pd.read_excel(ARCHIVO, sheet_name=0)
    print(df.columns)

    for index, fila in df.iterrows():
        cod_pdv = str(fila.get('COD_PDV', "") or "")
        nom_pdv = str(fila.get('NOM_PDV', "") or "")
        terminal = str(fila.get('TERMINAL', "") or "")

        print(f"{cod_pdv} - {nom_pdv} - {terminal}")
        crear_terminales(cod_pdv, nom_pdv, terminal)
