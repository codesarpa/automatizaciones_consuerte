...
def iniciar_sesion_superflex():
    ...
    try:
        ...
        log.error(f"Ocurrio un error al iniciar sesión es superflex: {e}")

def ingresar_menu_administrar_usuarios():
    try:
        page.wait_for_timeout(10000)
        page.locator("span.layout-menuitem-text", has_text="ADMINISTRACIÓN").click()
        page.wait_for_timeout(1000)  # Esperar un poco
        page.locator("span.layout-menuitem-text", has_text="USUARIOS").click()
        page.wait_for_timeout(1000)  # Esperar un poco
        page.locator("span.layout-menuitem-text", has_text="Administración de usuarios").click()
...
def ir_a_pagina(page, num_pagina):
    try:
        print(f"Intentando ir a la página {num_pagina}...")
        log.info(f"Intentando ir a la página {num_pagina}...")

        # Intenta hasta 200 veces avanzar si la página no está visible
        for _ in range(200):
            botones = page.locator("button.p-paginator-page")
            total_botones = botones.count()

            # Buscar si la página deseada está visible
            for i in range(total_botones):
                ...
                if texto == str(num_pagina):
                    ...
                    print(f"Página {num_pagina} abierta correctamente.")
                    log.info(f"Página {num_pagina} abierta correctamente.")
                    return True

            # Si no aparece, presionar el botón "siguiente"
            siguiente = page.locator("button.p-paginator-next")
            if siguiente.is_enabled():
                siguiente.click()
                page.wait_for_timeout(1000)
            else:
                print(f"Fin de paginación alcanzado antes de llegar a {num_pagina}.")
                log.info(f"Fin de paginación alcanzado antes de llegar a {num_pagina}.")
                return False

        print(f"No se encontró la página {num_pagina} después de varios intentos.")
        log.info(f"No se encontró la página {num_pagina} después de varios intentos.")
        return False

    except Exception as e:
        print(f"Error al intentar ir a la página {num_pagina}: {e}")
        log.info(f"Error al intentar ir a la página {num_pagina}: {e}")
        return False
...
def recorrer_usuarios():
    ...
    pagina_actual = 129  # contador manual de páginas
    ...
    while True:
        ...
        for i in range(total):
            ...
            if "45, 172, 49" in color:
                ...
                # Esperar hasta 10 segundos que se habilite el botón
                for _ in range(20):  # 20 x 0.5 = 10 segundos
                    ...
                    # Si después de 10 segundos sigue deshabilitado:
                    # Esperar a que desaparezca el loader (si existe)
                    try:
                        page.wait_for_selector("app-loader", state="detached", timeout=15000)
                    except:
                        print("El loader no desapareció a tiempo, intentando continuar de todas formas...")
                        log.info("El loader no desapareció a tiempo, intentando continuar de todas formas...")

                    # Ahora sí intentar hacer clic en 'Cancelar'
                    page.get_by_text("Cancelar").click()
                    time.sleep(2)
                    print("No se encuentra habilitado el botón actualizar.")
                    log.info("No se encuentra habilitado el botón actualizar.")
                    radio_button.click()
                ...
                # Validar si la actualización devolvió a la primera página
                try:
                    ...
                    if pagina_visible != pagina_actual:
                        print(f"Regresó a la página {pagina_visible}, volviendo a {pagina_actual}...")
                        ir_a_pagina(page, pagina_actual)
                        time.sleep(2)
                except Exception as e:
                    print(f"Error al validar la página activa: {e}")
            else:
        ...
        try:
            ...
            if next_button.is_enabled():
                next_button.click()
                pagina_actual += 1
                print(f"Avanzando a la página {pagina_actual}...")
